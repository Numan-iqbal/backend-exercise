...
describe('Test calculatePriceWithTaxes', () => {
  ...
  it('should calculate the correct import cost for each item', () => {
    ...
    expect(coffee?.cost).toBe(34720)

    expect(bigAvocados?.cost).toBe(0)
    expect(bigAvocados?.cost).toBe(500)

    expect(biggerAvocados?.cost).toBe(0)
    expect(biggerAvocados?.cost).toBe(1000)

    expect(tesla?.cost).toBe(2880000)
    expect(tesla?.cost).toBe(26880000)

    expect(schnitzel?.cost).toBe(3000)
    expect(schnitzel?.cost).toBe(53000)
  })
})
